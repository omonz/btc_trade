...
class DashboardController extends Controller
{
    ...
    public function depositMethod()
    {
        ...
        $data['gateways'] = PaymentMethod::where('id','<',100)->get();
    ...
    public function submitBankDeposit(Request $request)
    {
        ...
        $in = Input::except('_method','_token');
        $in['status'] = $request->status == 'on' ? '1' : '0';
        $in['id'] = PaymentMethod::where('id','>',100)->orderBy('id','desc')->first()->id + 1;
            if($request->hasFile('image')){
            $image3 = $request->file('image');
            $filename3 = time().'h7'.'.'.$image3->getClientOriginalExtension();
            $location = 'assets/images/' . $filename3;
    ...
    public function showBank()
    {
        ...
        $data['gateways'] = PaymentMethod::where('id','>',100)->orderBy('id','asc')->get();
    ...
    /**
     * Admin confirm user request and confirm the deposit made
    ...
    */
    ...
    /**
     * this is where the confirm withdraw is coming from
     * to copy and create the confirm deposit section
     */

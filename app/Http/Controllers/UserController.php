...
class UserController extends Controller
{
    ...
    public function submitDepositFund(Request $request)
    {
        ...
        if ($pay_id == 1) {
            ...
            $gateway = PaymentMethod::whereId(2)->first();
            $charge = ($gateway->fix + ( $amount*$gateway->percent / 100 ));
            $lo['usd'] = round(($amount + $charge) / $gateway->rate,2);
        }elseif ($pay_id == 3) {
            $gateway = PaymentMethod::whereId(3)->first();
            $charge = ($gateway->fix + ( $amount*$gateway->percent / 100 ));
            $all = file_get_contents("https://blockchain.info/ticker");
            $res = json_decode($all);
            $lo['btc_amo'] = round( ($amount + $charge) * (1/$res->USD->last),8);
            $lo['usd'] = round(($amount + $charge),2);
        }elseif ($pay_id == 4) {
            $gateway = PaymentMethod::whereId(4)->first();
            $charge = ($gateway->fix + ( $amount*$gateway->percent / 100 ));
            $lo['usd'] = round(($amount + $charge) / $gateway->rate,2);
        }elseif ($pay_id == 5) {
            $gateway = PaymentMethod::whereId(5)->first();
            $charge = ($gateway->fix + ( $amount*$gateway->percent / 100 ));
            $all = file_get_contents("https://blockchain.info/ticker");
            $res = json_decode($all);
            $lo['btc_amo'] = round( ($amount + $charge) * (1/$res->USD->last),8);
            ...
            $gateway = PaymentMethod::whereId(101)->first();
            $charge = ($gateway->fix + ( $amount*$gateway->percent / 100 ));
            $all = file_get_contents("https://blockchain.info/ticker");
            $res = json_decode($all);
            $lo['btc_amo'] = round( ($amount + $charge) * (1/$res->USD->last),8);
    ...
    public function depositPreview($id)
    {
        ...
        $data['bank'] = PaymentMethod::orderBy('id','asc')->where('id', '>', '8')->whereStatus(1)->get();
